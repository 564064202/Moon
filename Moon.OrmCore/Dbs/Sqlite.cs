...
namespace Moon.Orm
{
	...
	public class Sqlite:Db
	{
		...
		public Sqlite(string linkString):base(linkString){
            //
            try
            {
                this.AdoMethod = new AdoSqlite(this.LinkString);
            }catch(Exception ex)
            {
                throw ex.InnerException;
            }
			_conn=AdoMethod.CreateConnection() as SQLiteConnection;
			//
		...
		public override object Add(EntityBase entity)
		{
			...
			var cmd=AdoMethod.CreateDbCommand() as SQLiteCommand;
		...
		public override DictionaryList ExecuteProToDictionaryList(String procName,params DbParameter[] parameters){
			var procDbCommand = AdoMethod.CreateDbCommand() as SQLiteCommand;
			procDbCommand.Connection=_conn;
			if (TransactionEnabled) {
				procDbCommand.Transaction=this.Transaction as SQLiteTransaction;
			}
			...
			if (parameters!=null&&parameters.Length>0) {
				...
				{
					procDbCommand.Parameters.Add(parameter as SQLiteParameter);
				}
			}
			DictionaryList list=new DictionaryList();
		...
		public override string ExecuteSqlToJson(string sql, params object[] values)
		{
			...
			//
			SQLiteCommand cmd = AdoMethod.CreateDbCommand() as SQLiteCommand;
			cmd.Connection=_conn;
			if (TransactionEnabled) {
				cmd.Transaction=this.Transaction as SQLiteTransaction;
			}

...
namespace Moon.Orm
{
    ...
    public class Oracle : Db
    {
        ...
        public Oracle(string linkString) : base(linkString)
        {
            //
            try
            {
                this.AdoMethod = new AdoOracle(this.LinkString);
            }
            catch (Exception ex)
            {
            ...
            _conn = this.Connection as OracleConnection;
            this.PName = ":";
        ...
        public override object Add(EntityBase entity)
        {
            ...
            var cmd = AdoMethod.CreateDbCommand() as OracleCommand;
            cmd.Connection = _conn;
            if (TransactionEnabled)
            {
                cmd.Transaction = this.Transaction as OracleTransaction;
            }
            int index = 1;
            sb.AppendLine();
            foreach (KeyValuePair<string, object> parameter in entity.ChangedMap)
            {
                ...
                cmd.Parameters.Add(this.PName + "p" + index, parameter.Value);
                index++;
            }
        ...
        public override List<T> GetOwnList<T>(MQLBase mql)
        {
            ...
            {
                cmd.Parameters.Add(this.PName + "p" + index, parameter);
                index++;
            }
        ...
        public override int Update(EntityBase entity)
        {
            ...
            {
                cmd.Parameters.Add(this.PName + "p" + index, a);
                index++;
            }
            cmd.CommandText = sb.ToString();
            //
        ...
        public override long GetCount<T>(WhereExpression expression)
        {
            ...
            {
                index++;
                cmd.Parameters.Add(PName + "p" + index, a);
            }
        ...
        public override DictionaryList GetDictionaryList(MQLBase mql)
        {
            ...
            foreach (var parameter in mql.Parameters)
            {
                cmd.Parameters.Add(PName + "p" + index, parameter);
                index++;
            }
            cmd.CommandText = sql2;
            if (DebugEnabled)
        ...
        {
            var procDbCommand = AdoMethod.CreateDbCommand() as OracleCommand;
            procDbCommand.Connection = _conn;
            if (TransactionEnabled)
            {
                procDbCommand.Transaction = this.Transaction as OracleTransaction;
            }
            ...
            if (parameters != null && parameters.Length > 0)
            {
                ...
                {
                    procDbCommand.Parameters.Add(parameter as OracleParameter);
                }
            }
            DictionaryList list = new DictionaryList();
        ...
        public override int ExecuteSqlWithNonQuery(string sql, params object[] values)
        {
            ...
            if (values != null && values.Length > 0)
            {
                ...
                {
                    index2++;
                    cmd.Parameters.Add(PName + "p" + index2, obj);
                }
            }
            //
            if (index != index2)
        ...
        public override DataSet GetPagerToDataSet(MQLBase mql, out int sumPageCount, out int sumDataCount, int pageIndex, int onePageDataCount, string oneOrderbyFieldName)
        {
            ...
            long start=(pageIndex-1)*onePageDataCount+1;
            long end = pageIndex * onePageDataCount;
            StringBuilder query = new StringBuilder();
            query.Append("SELECT * FROM ( SELECT T_MOON_Search.*, ROWNUM RN  FROM (");
            query.Append(sql);
            query.Append(" ) T_MOON_Search WHERE ROWNUM <= " + end + ") ");
            query.Append(" WHERE RN >=" + start);
            sql = query.ToString();
            if (DebugEnabled)
            {
                CurrentSQL = sql;
